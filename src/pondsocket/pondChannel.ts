import {
    Anything, BaseClass, BasePromise, default_t, PondBase, PondError, PondPath, RejectPromise, Subscription
} from "../pondbase";
...
export class PondChannel extends BaseClass {
    ...
    public addUser(user: SocketCache, channelName: string, joinParams: default_t): Promise<void> {
        return BasePromise<void, { channelName: string }>({channelName}, async (resolve, reject) => {
            ...
            if (!resolved) return reject("Invalid channel name", 400);

            if (!channel) channel = await this._createChannel(channelName);
            ...
            const request: IncomingJoinMessage = {
                joinParams, ...resolved, clientId: user.clientId, channelName, clientAssigns: user.assigns
            };

            const resolver = (data: ResponseResolver) => {
                if (data.error) return reject(data.error.errorMessage, data.error.errorCode);
                ...
                if (data.message) channel.sendTo(data.message.event, data.message.payload, PondSenders.POND_CHANNEL, [user.clientId]);
    ...
    public subscribe(channelName: string, callback: Subscriber): Subscription {
        ...
        if (channel) return channel.doc.subscribe(callback);
    ...
    public removeUser(clientId: string): void {
        if (this._subscriptions[clientId]) {
            ...
            for (const channel of this._channels.generate()) if (channel.hasUser(clientId)) channel.removeUser(clientId);
        }
    }
    ...
    private _execute<A>(channelName: string, handler: ((channel: Channel) => A)) {
        ...
        if (newChannel) return handler(newChannel);
        ...
        return (data: ChannelEvent) => {
            ...
            if (info) {
                ...
                const resolver = (innerData: ResponseResolver) => {
                    ...
                    if (innerData.error) returnVal = new PondError(innerData.error.errorMessage, innerData.error.errorCode, {
                        event: data.event, channelName: data.channelName
                    });

                    else {
                        if (!this.isObjectEmpty(channelData)) data.channel.data = channelData;

                        if (!Object.values(PondSenders).includes(data.clientId as any)) {
                            data.channel.updateUser(data.clientId, presence, assigns);

                            if (innerData.message) data.channel.sendTo(innerData.message.event, innerData.message.payload, PondSenders.POND_CHANNEL, [data.clientId]);
                        }
                    }
                };
