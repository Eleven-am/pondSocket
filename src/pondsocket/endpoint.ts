...
import {
    BaseClass, BasePromise, default_t, PondBase, PondDocument, PondError, PondPath, Resolver, ResponsePicker
} from "../pondbase";
...
export class Endpoint extends BaseClass {
    ...
    public authoriseConnection(request: IncomingMessage, socket: internal.Duplex, head: Buffer, data: Resolver) {
        return BasePromise<void, internal.Duplex>(socket, async (resolve, reject) => {
            ...
            const req: IncomingConnection = {
                headers: request.headers, ...data, clientId: doc.id
            };
    ...
    public closeConnection(clientId: string): void {
        const message: ServerMessage = {
            action: ServerActions.CLOSE, channelName: "SERVER", event: "CLOSED_FROM_SERVER", payload: {}
        };
    ...
    public send(clientId: string | string[], event: string, message: default_t): void {
        const newMessage: ServerMessage = {
            action: ServerActions.MESSAGE, channelName: PondSenders.ENDPOINT, event, payload: message
        };
    ...
    private _manageSocket(cache: PondDocument<Omit<SocketCache, "clientId">>) {
        ...
        socket.addEventListener("close", () => {
            for (const channel of this._channels.generate()) channel.removeUser(cache.id);
            cache.removeDoc();
        });
    ...
    private async _readMessage(cache: PondDocument<Omit<SocketCache, "clientId">>, message: string) {
        const errorMessage: ServerMessage = {
            action: ServerActions.ERROR, event: "INVALID_MESSAGE", channelName: PondSenders.ENDPOINT, payload: {}
        };
        ...
        try {
            ...
            else await this._handleMessage(cache, data);

            if (!this.isObjectEmpty(errorMessage.payload)) Endpoint._sendMessage(cache.doc.socket, errorMessage);
            ...
            if (e instanceof SyntaxError) {
                ...
                const message: ServerMessage = {
                    ...
                    payload: {
                        message: e.errorMessage, code: e.errorCode, data: e.data.event ? undefined : e.data
                    }
                };
    ...
    private async _handleMessage(cache: PondDocument<Omit<SocketCache, "clientId">>, message: ClientMessage) {
        switch (message.action) {
            case "JOIN_CHANNEL":
                ...
                } else throw new PondError("The channel was not found", 4004, {
            ...
            case "SEND_MESSAGE_TO_USER":
                await this._channelAction(message.channelName, message.event, async (channel) => {
                    if (!message.addresses || message.addresses.length === 0) throw new PondError("No addresses provided", 400, {
    ...
    private _channelAction(channelName: string, event: string, action: (channel: Channel) => Promise<void> | void) {
        ...
        if (!channel) throw new PondError("Channel not found", 404, {channelName, event});
