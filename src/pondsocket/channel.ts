import {
    Anything, BaseClass, Broadcast, PondBase, PondDocument, PondError, RejectPromise, Subscription
} from "../pondbase";
import {
    NewUser, PondAssigns, PondChannelData, PondMessage, PondPresence, ServerMessage
} from "./types";
...
export class Channel extends BaseClass {
    ...
    public addUser(user: NewUser): void {
        ...
        this._broadcast.publish({
            ...
            clientPresence: {...user.presence, id: clientId}
        });
    }
    ...
    public broadcast(event: string, message: PondMessage, sender: PondSenders | string = PondSenders.POND_CHANNEL): void {
        ...
        if (!client && !Object.values(PondSenders).includes(sender as any)) throw new PondError('Client not found', 5002, sender);
    ...
    public updateUser(clientId: string, presence: PondPresence, assigns: PondAssigns) {
        ...
        if (client) {
            this._channelAssigns[clientId] = {
                ...client.assigns, ...assigns, presenceId: client.presence.id
            };
